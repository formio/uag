...
## Remaining fields for <%= parentLabel %>:
<%= fields %>
Use the `collect_field_data` tool to provide values for the remaining fields for <%= parentLabel %>. You can collect multiple fields at once by providing an array of field updates.

<%= dataSummary %>

**Next Steps**:
1. Ask the user for any remaining fields for <%= parentLabel %>.
