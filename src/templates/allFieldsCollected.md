...
2. If no other optional fields need to be provided, and there is another row of information is needed for this table, then use the `collect_field_data` tool to collect any additional required rows.
3. If no additionial required rows are needed, then ask the user if they wish to "Add Another" (add another row to this parent component). If they affirm, then use the `collect_field_data` tool to collect the next row's data. Make sure to increment the row index for the fields within this parent component (e.g. <%= parent.data_path %>[<%= rowIndex %>] => <%= parent.data_path %>[<%= (rowIndex + 1) %>]) when collecting data for the next row.
