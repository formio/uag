...
export const findSubmission = async (project: UAGProjectInterface): Promise<ToolInfo> => {
    return defaultsDeep(project.config?.toolOverrides?.find_submission_by_field || {}, {
        ...
        inputSchema: {
            ...
            submission_id_partial: z.string().optional().describe('Optional: Last 4 characters of a specific submission ID to update when multiple matches are found')
        },
        execute: async ({ form_name, search_query, fields_requested, limit = 10, submission_id_partial }: any, extra: any) => {
